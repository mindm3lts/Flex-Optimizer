...
const getPackageIcon = (packageType: PackageType) => {
    switch (packageType) {
        case "Box":
        case "Custom Sized":
            return <BoxIcon className="w-6 h-6 text-cyan-600 dark:text-cyan-400" />;
        case "Envelope":
            return <EnvelopeIcon className="w-6 h-6 text-cyan-600 dark:text-cyan-400" />;
        case "Plastic Bag":
            return <PackageIcon className="w-6 h-6 text-cyan-600 dark:text-cyan-400" />;
        default:
            return <PackageIcon className="w-6 h-6 text-gray-400 dark:text-gray-500" />;
    }
}
...
const SummaryItem: React.FC<{ icon: React.ReactNode; label: string; value: string | number; isLoading: boolean }> = ({ icon, label, value, isLoading }) => (
  <div className="flex flex-col items-center justify-center p-2 bg-gray-100 dark:bg-gray-700/50 rounded-lg text-center h-full">
    {isLoading ? (
      <>
        <div className="w-6 h-6 bg-gray-300 dark:bg-gray-600 rounded-full animate-pulse"></div>
        <div className="h-4 w-16 bg-gray-300 dark:bg-gray-600 rounded mt-2 animate-pulse"></div>
        <div className="h-3 w-12 bg-gray-300 dark:bg-gray-600 rounded mt-1 animate-pulse"></div>
      </>
    ) : (
      <>
...
const LocationStopDisplay: React.FC<{ stop: RouteStop }> = ({ stop }) => (
  <div className="relative flex items-start bg-blue-100 dark:bg-blue-900/50 border border-blue-200 dark:border-blue-700 p-3 rounded-md">
    <MyLocationIcon className="w-6 h-6 text-blue-500 dark:text-blue-400 mr-3 mt-1 flex-shrink-0" />
    <div className="flex-grow">
      <div className="flex justify-between items-center text-xs font-mono mb-1">
        <span className="font-bold text-blue-600 dark:text-blue-300 text-sm">START: #{1}</span>
      </div>
      <p className="font-semibold text-gray-800 dark:text-gray-100">{stop.street}</p>
      <p className="text-sm text-gray-600 dark:text-gray-400">{stop.city}</p>
    </div>
  </div>
);
...
export const RouteDisplay: React.FC<RouteDisplayProps> = ({ route, summary, trafficInfo, onRouteUpdate, onAiOptimize, isOptimizing }) => {
  ...
  return (
    <div className="mt-6 bg-white dark:bg-gray-800 rounded-lg p-4 shadow-lg animate-fade-in flex flex-col flex-grow">
      <div className="mb-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3">
          <h2 className="text-xl font-bold text-cyan-600 dark:text-cyan-400 text-center mb-3">Optimize Your Route</h2>
          
          <div className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 p-2 rounded-md mb-2">
              <label htmlFor="useLocationToggle" className="flex items-center flex-grow text-sm font-medium text-gray-700 dark:text-gray-200">
                <MyLocationIcon className="w-5 h-5 mr-2 text-blue-500 dark:text-blue-400"/> Start from my location
              </label>
              <button
                ...
                className={`${useLocation ? 'bg-blue-600' : 'bg-gray-400 dark:bg-gray-600'} relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-blue-500`}
           ...
           <div className="flex items-center justify-between bg-gray-100 dark:bg-gray-800 p-2 rounded-md mb-3">
              <label htmlFor="avoidLeftTurnsToggle" className="flex items-center flex-grow text-sm font-medium text-gray-700 dark:text-gray-200">
                <NoLeftTurnIcon className="w-5 h-5 mr-2 text-yellow-500 dark:text-yellow-400"/> Prefer fewer left turns
              </label>
              <button
                ...
                className={`${avoidLeftTurns ? 'bg-yellow-500' : 'bg-gray-400 dark:bg-gray-600'} relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-yellow-500`}
          ...
          <div className="grid grid-cols-2 gap-2">
             <button
                ...
                className="col-span-2 flex items-center justify-center px-3 py-3 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-md transition-colors disabled:bg-gray-500 dark:disabled:bg-gray-600 disabled:cursor-not-allowed"
              ...
              <button
                ...
                disabled={isOptimizing}
                className="col-span-2 flex items-center justify-center px-3 py-2 bg-gray-300 hover:bg-gray-400 text-gray-700 dark:bg-gray-600 dark:hover:bg-gray-700 dark:text-gray-200 text-sm font-semibold rounded-md transition-colors disabled:opacity-50"
          ...
          >
            ...
            <div className="flex-grow ml-3">
              ...
              <div className="mt-3 flex items-center space-x-3 flex-wrap gap-y-2">
                {stop.isPriority && (
                    <div className="flex items-center text-yellow-800 bg-yellow-100 dark:text-yellow-300 dark:bg-yellow-900/50 px-2 py-1 rounded-md" title="Priority Delivery">
                        <PriorityIcon className="w-4 h-4 mr-1" />
                        <span className="text-xs font-bold uppercase">Priority</span>
                    </div>
                )}
                 {stop.deliveryWindowEnd && (
                     <div className="flex items-center text-red-800 bg-red-100 dark:text-red-400 dark:bg-red-900/50 px-2 py-1 rounded-md" title={`Deliver by ${formatTimeToAMPM(stop.deliveryWindowEnd)}`}>
                        <ClockIcon className="w-4 h-4 mr-1" />
                        <span className="text-xs font-medium">by {formatTimeToAMPM(stop.deliveryWindowEnd)}</span>
                    </div>
                )}
                 <div className="flex items-center text-gray-700 bg-gray-200 dark:text-gray-300 dark:bg-gray-800 px-2 py-1 rounded-md" title={`Stop Type: ${stop.stopType}`}>
                    {getStopTypeIcon(stop.stopType)}
                    <span className="text-xs font-medium">{stop.stopType}</span>
                </div>
                 <button onClick={() => handleOpenConstraintModal(stop, index)} className="flex items-center text-xs text-cyan-700 hover:text-cyan-600 bg-cyan-100 hover:bg-cyan-200 dark:text-cyan-400 dark:hover:text-cyan-300 dark:bg-cyan-900/50 dark:hover:bg-cyan-900 font-semibold py-1 px-2 rounded-md transition-colors">
              ...
              <textarea
                ...
                rows={2}
                className="mt-3 w-full bg-gray-200 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1.5 text-sm text-gray-800 dark:text-gray-200 placeholder-gray-500 focus:ring-cyan-500 focus:border-cyan-500 resize-y"
              />

              <div className="mt-3 pt-3 border-t border-gray-300 dark:border-gray-600/50">
                <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Package Details</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                   <div className="flex items-center space-x-2 bg-gray-200 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1.5">
                      ...
                      <select
                          ...
                          className="bg-transparent w-full text-gray-800 dark:text-gray-200 focus:outline-none"
                      >
                   ...
                   <input
                    ...
                    className="bg-gray-200 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1.5 text-gray-800 dark:text-gray-200 placeholder-gray-500 focus:ring-cyan-500 focus:border-cyan-500"
                  />
                   <input
                    type="text"
                    value={stop.tba}
                    ...
                    placeholder="TBA..."
                    className="md:col-span-2 bg-gray-200 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1.5 text-gray-800 dark:text-gray-200 placeholder-gray-500 focus:ring-cyan-500 focus:border-cyan-500"
                  />
                </div>
              </div>
            </div>
            <button
              ...
              className="ml-2 p-1 text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-500 transition-colors flex-shrink-0"
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
       ...
       <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        <h3 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3 text-center">Live Traffic</h3>
        <div className="space-y-3">
             {isTrafficLoading ? (
                <div className="p-3 bg-gray-100 dark:bg-gray-700/50 rounded-lg animate-pulse h-20" />
            ) : trafficInfo && (
                    ...
                    <div className="flex items-start">
                        ...
                        <div className="flex-grow">
                            <div className="flex justify-between items-center">
                                ...
                                <p className="text-xs text-gray-400 dark:text-gray-500">As of {trafficInfo.lastUpdated}</p>
                            </div>
                            <p className="text-sm text-gray-700 dark:text-gray-300 mt-1">{trafficInfo.summary}</p>
                        </div>
                    </div>
                </div>
            )}
      ...
      {stopToDeleteIndex !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-75 flex items-center justify-center z-50 animate-fade-in" aria-modal="true" role="dialog" aria-labelledby="delete-modal-title">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-sm mx-4">
             <div className="sm:flex sm:items-start">
                <div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 dark:bg-red-900/50 sm:mx-0 sm:h-10 sm:w-10">
                    <WarningIcon className="h-6 w-6 text-red-500 dark:text-red-400" aria-hidden="true" />
                </div>
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white" id="delete-modal-title">Confirm Deletion</h3>
                    <div className="mt-2">
            ...
            <div className="mt-6 flex justify-end space-x-3">
              <button onClick={handleCancelDelete} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-600 dark:hover:bg-gray-700 dark:text-gray-200 font-semibold rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">Cancel</button>
      ...
      {isMapsModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-75 flex items-center justify-center z-50 animate-fade-in" aria-modal="true" role="dialog" aria-labelledby="maps-modal-title">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-sm mx-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white" id="maps-modal-title">Split Route for Google Maps</h3>
              <button onClick={() => setIsMapsModalOpen(false)} aria-label="Close modal" className="p-1 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white"><CloseIcon className="w-6 h-6" /></button>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-300 mt-2 mb-4">Your route is too long for a single link. Use the buttons below to open it in chunks.</p>
            <div className="space-y-3">
      ...
      {editingStop && (
         <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-75 flex items-center justify-center z-50 animate-fade-in" aria-modal="true" role="dialog" aria-labelledby="constraint-modal-title">
             <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-sm mx-4">
                 <div className="flex justify-between items-center">
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white" id="constraint-modal-title">Edit Stop Details</h3>
                    <button onClick={() => setEditingStop(null)} aria-label="Close modal" className="p-1 text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white"><CloseIcon className="w-6 h-6" /></button>
                 </div>
                 <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">For stop #{editingStopIndex !== null ? editingStopIndex + (locationStop ? 2: 1) : ''}</p>
                 <div className="mt-4 space-y-4">
                    <label htmlFor="isPriorityToggle" className="flex items-center justify-between cursor-pointer">
                        <span className="flex items-center text-gray-800 dark:text-gray-200">
                          <PriorityIcon className="w-5 h-5 mr-2 text-yellow-500 dark:text-yellow-400" />
                          Priority Delivery
                        </span>
                        <button
                            id="isPriorityToggle"
                            ...
                            className={`${editingStop.isPriority ? 'bg-yellow-500' : 'bg-gray-400 dark:bg-gray-600'} relative inline-flex items-center h-6 rounded-full w-11 transition-colors`}
                    ...
                    <div className="space-y-1">
                        <label htmlFor="stopType" className="flex items-center text-gray-800 dark:text-gray-200 mb-1">
                             <ApartmentIcon className="w-5 h-5 mr-2 text-gray-500 dark:text-gray-400"/>
                             Stop Type
                        </label>
                         <select
                            id="stopType"
                            ...
                            className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 text-gray-800 dark:text-gray-200 focus:ring-cyan-500 focus:border-cyan-500"
                         >
                    ...
                    <div className="space-y-1">
                        <label htmlFor="deliveryTime" className="flex items-center text-gray-800 dark:text-gray-200">
                             <ClockIcon className="w-5 h-5 mr-2 text-red-500 dark:text-red-400"/>
                             Deliver By
                        </label>
                         <input
                            id="deliveryTime"
                 ...
                 <div className="mt-6 flex justify-end space-x-3">
                    <button onClick={() => setEditingStop(null)} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-600 dark:hover:bg-gray-700 dark:text-gray-200 font-semibold rounded-md transition-colors">Cancel</button>
