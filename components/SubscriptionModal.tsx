...
export const SubscriptionModal: React.FC<SubscriptionModalProps> = ({ user, onClose, onSubscriptionChange }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 animate-fade-in" aria-modal="true" role="dialog">
      <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-xl p-6 w-full max-w-md mx-4 relative">
        <button onClick={onClose} aria-label="Close modal" className="absolute top-4 right-4 p-1 text-gray-400 hover:text-white">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            {/* Free Plan */}
            <div className={`p-4 rounded-lg border-2 ${!isPro ? 'border-cyan-500' : 'border-gray-700'}`}>
            ...
            {/* Pro Plan */}
             <div className={`p-4 rounded-lg border-2 ${isPro ? 'border-yellow-500' : 'border-gray-700'} bg-gray-900/50`}>
                <h4 className="font-bold text-lg text-yellow-400">Pro Plan</h4>
                 <p className="text-2xl font-bold text-white my-2">$1.99 <span className="text-base font-normal text-gray-400">/ month</span></p>
                <ul className="mt-3 space-y-2 text-gray-300">
        ...
        <div className="mt-6">
          {isPro ? (
            ...
            <button
              ...
              className="w-full bg-yellow-600 hover:bg-yellow-700 text-gray-900 font-bold py-3 px-4 rounded-lg transition duration-300 transform hover:scale-105"
            >
              Upgrade to Pro
            </button>
          )}
